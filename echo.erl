...
%Nicholas Cali and Giovanna Savatta
start() ->
...
loop(noninitiator,NPIDs,Rec,Parent,Tree) ->  
    ...
    {From, _W} ->
        if 
        length(NPIDs) == Rec+1 -> %got replies from all neighbors, send parent
            [NPIDs!{Parent, parent_of, self() | W++Tree}];
        true ->             %wait for messages
            loop(noninitiator, NPIDs, Rec, Parent, [W++Tree})
        end
    end;
...
loop(initiator,NPIDs,Rec,undef,Tree) ->  
    ...
    {From, _W} ->
        ...
        true -> %wait for messages
            loop(noninitiator, NPIDs, Rec, From, [W++Tree])
        end
    end;
